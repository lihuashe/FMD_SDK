...
void my_adc_init(void)
{
    ...
    // JL_GPADC->CON = clk_div | BIT(3);
}

/*
 * @brief 获取通道电压值（自定义
 * @param ch:通道号
 * @return 当前通道ad数字量（采4次平均），mV
*/
static u16 data1,data2,data3,data4;
int my_adc_scan_more(u8 ch)
{
    if ((adc_info[ch].ch >> 6) == 0b11) {
        log_info("ch reuse & busy\n");
        return -1;    //ch reuse & busy
    }
    kick_start_flg = 1;
    u8 adc_ch = (ch >= ADC_CH_P33_TEST) ? (ADC_CH_P33_TEST) : ch;
    ...
    JL_GPADC->CON = clk_div | ((adc_ch & 0xf) << 8) | BIT(3) | BIT(6);   /*ch选择 //  adc模拟模块使能打开 // 配置分频*/
    //
    JL_GPADC->CON |= BIT(4);//adc控制器en
    JL_GPADC->CON |= BIT(6);//clear pnd,且对该bit写‘1’将启动 ADC 转换
    while (!(JL_GPADC->CON & BIT(7)));
    data1 = JL_GPADC->DAT;
    //
    JL_GPADC->CON |= BIT(6);//clear pnd,且对该bit写‘1’将启动 ADC 转换
    ...
    data4 = JL_GPADC->DAT;
    JL_GPADC->CON |= BIT(6);//clear pnd
    u16 data = (data1+data2+data3+data4) >> 2;
    return data;
}
...
int my_adc_scan(u8 ch)
{
    ...
    JL_GPADC->CON = clk_div | ((adc_ch & 0xf) << 8) | BIT(3) | BIT(6);   /*ch选择 //  adc模拟模块使能打开 */     //OK!
 ...
 * @return 电压值，mV
*/
u32 my_adc_to_voltage(u32 value)
{
    u32 adc_vbg = my_adc_scan(ADC_CH_PMU_VBG08);
    u32 adc_vbat = value;
    return adc_value_to_voltage(adc_vbg, adc_vbat);
}
 ...
 * @time:
*/
u32 my_adc_get_volgate(u8 ch)
{
    u32 adc_vbg = my_adc_scan(ADC_CH_PMU_VBG08);
    u32 adc_vbat = my_adc_scan_more(ch);
    return adc_value_to_voltage(adc_vbg, adc_vbat);
}

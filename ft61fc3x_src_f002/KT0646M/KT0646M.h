...
//  V0.5    2016-10-11  修改了初始化函数里的一些配置，把0x25,0x26寄存器的配置加到了初始化函数里面。
...
//  V0.5.3  2016-12-15  在初始化时把lo_fine_vref_sel改为3，不用在tune台的时候再来回修改了.为了温度
...
//  V1.3    2017-09-18	COMPEN_GAIN从3改成1,MIC_SENS_GAIN从9改成5,COMPANDOR_TC_48ms改成COMPANDOR_TC_12ms
...
#define TEST_CHA_FREQ 621130  // 测试模式下CHA 频率
#define TEST_CHB_FREQ 659130 // 测试模式下CHB 频率
...
#define    GAIN_SEL                 1        //PGA增益：0:为-6dB，1:0dB,2:6dB,3:12dB
...
#define    ECHO_GAINDOWN            0        //输入信号进行缩放：0-为-13dB
...
如果选的是软拐点，这ALC_VMAX的拐点选择小于等于0.3045，查datasheet得到ALC_VMAX应配置为72
如果选的是硬拐点，这ALC_VMAX的拐点选择小于等于0.3045/1.5=0.203，查datasheet得到ALC_VMAX应配置为54
***************************************************************************/
...
// void Delay_ms(uint16_t iTime_Number);

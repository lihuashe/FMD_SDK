...
//函数说明：
//全局变量：无
//输    入：UINT8 Register_Address, uint16_t Word_Data
//返    回：无
...
{
    uint8_t reg_value_l = (uint8_t)(Word_Data & 0x00FF);
    uint8_t reg_value_h = (uint8_t)(Word_Data >> 8 & 0x00FF);
    IIC_Start();
    IIC_Send_Byte(KTWirelessMicTxw_address);
    if (IIC_Wait_Ack()) {
        // to-do: 设备无响应
        goto _end;
    }
    ...
    IIC_Send_Byte(reg_value_h);
    if (IIC_Wait_Ack()) {
        // to-do: iic通讯失败
        goto _end;
    }
    IIC_Send_Byte(reg_value_l);
    if (IIC_Wait_Ack()) {
        // to-do: iic通讯失败
        goto _end;
    }
...
{   
    ...
    IIC_Start();
    IIC_Send_Byte(KTWirelessMicTxr_address);
    if (IIC_Wait_Ack()) {
        // to-do: 设备无响应
        goto _end;
    }

    reg_value = IIC_Read_Byte();    // reg value high 8 bits
    reg_value = reg_value << 8;
    IIC_Ack();

    reg_value |= IIC_Read_Byte();  // reg value low 8 bits
    IIC_NAck();
...
//版    本：V1.0
//-----------------------------------------------------------------------------
uint16_t KT_WirelessMicTx_PreInit(void)
{   
    KT_Bus_Init();
...
bool KT_WirelessMicTx_Init(void)
{
    ...
    KT_WirelessMicTx_Mic_Sens(MIC_SENS_GAIN_6);
...
uint8_t KT_WirelessMicTx_Read_Mic_Sens(void)
{
    uint16_t reg1C;
...
//-----------------------------------------------------------------------------
//lopa_div1 * lopa_div2
static const uint8_t lopa_div_tab[13]=
{
    6, 8, 10, 12,
      16, 20, 24,
      32, 40, 48,
...
static const uint8_t lopa_div_tab2[13][2]=
{
     {0,0},{0,1}, {0,2}, {0,3},
           {1,1}, {1,2}, {1,3},
           {2,1}, {2,2}, {2,3},
...
bool KT_WirelessMicTx_Tune_Direct(uint16_t chan_rega, uint16_t chan_regb)
{
    uint16_t regx,regy;
    uint16_t state;
    uint8_t retry;
...
//    KT_Bus_Write(0x2d, (regx&0xfff8)|0x0003); //lo_fine_vref_sel
    ...
    KT_Bus_Write(0x08, chan_rega);
    KT_Bus_Write(0x09, chan_regb);
...
//    KT_Bus_Write(0x2d, (regx&0xfff8)); //lo_fine_vref_sel=0
    ...
    if (regx == 0) {
        //to-do: pll not ready after second tune!
        // log_info("pll not ready after second tune!\r\n");
        return false;
    }
#endif
